...
## T-1163d - Native Video Presenter speech animation adapter - TODO
- Add a provider-neutral animation contract that consumes a ready private normalized base plus an owned speech track, with LatentSync as the first backend.
- Keep animation explicit and opt-in. Claim work before GPU execution, persist an idempotent cache key, bound duration/cost, and never expose model names or private storage paths.
- Fallback must preserve the normalized still/talking-visual path without losing user assets. No Product Ad default switch until one capped private QA passes.
