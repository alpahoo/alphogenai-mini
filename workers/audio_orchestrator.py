...
async def example_worker_with_audio():
    """
    Example showing how to integrate audio into the worker pipeline.
    """
    from supabase_client import SupabaseClient
    from runway_orchestrator import RunwayOrchestrator
    
    supabase = SupabaseClient()
    runway = RunwayOrchestrator()
    audio = AudioOrchestrator()
    
    job_id = "example-job-id"
    user_id = "example-user-id"
    prompt = "A whale rescue operation in the ocean with dramatic waves"
    
    try:
        logger.info(f"Stage 1: Generating video for job {job_id}")
        video_result = await runway.run(job_id, user_id, prompt)
        
        if video_result["status"] != "success":
            raise Exception(f"Video generation failed: {video_result.get('error')}")
        
        video_url = video_result["video_url"]
        duration = video_result.get("duration", 60.0)
        
        await supabase.update_job_state(
            job_id,
            app_state={"stage": "audio_generation"},
            video_url=video_url,
            current_stage="audio_generation"
        )
        
        logger.info(f"Stage 2: Generating audio for job {job_id}")
        ...
        await supabase.update_job_state(
            job_id,
            app_state={"stage": "completed", "audio_result": audio_result},
...
if __name__ == "__main__":
    ...
    asyncio.run(example_worker_with_audio())
