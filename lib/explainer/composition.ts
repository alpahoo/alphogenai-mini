/**
 * Explainer composition builder — pure, deterministic, no I/O, no network.
 *
 * This is the SHARED source of the explainer's visual templates. It is a faithful
 * TypeScript port of the composition logic in
 * `infra/explainer-renderer/build.js` (the standalone Node renderer shipped to
 * Modal/VPS): same 6 fixed templates, same GSAP timeline, byte-identical HTML for
 * the same input.
 *
 * Why a port and not an import: build.js runs as a standalone Node script inside
 * the Modal image (`node /app/build.js`), outside the Next.js bundle, so it cannot
 * import a TS module without changing how the render image ships. Keeping a faithful
 * TS copy lets the in-app Explainer Studio render a low-fi, WYSIWYG **preview**
 * (iframe srcdoc + GSAP play) at $0 with no Modal call, while build.js keeps owning
 * the final render unchanged. The templates are intentionally kept identical between
 * the two; see docs/product/t1120-preview-spike.md.
 *
 * The LLM never writes this code — it only produces the storyboard DATA. These are
 * FIXED, parameterized templates (the production-safe model).
 */

import type { ExplainerScene, ExplainerStoryboard } from "./storyboard";

export const FPS = 30;
export const COMPOSITION_WIDTH = 1920;
export const COMPOSITION_HEIGHT = 1080;

/** Per-scene assets resolved at render/preview time (screenshot, voice-over). */
export interface CompositionAssets {
  screenshot_url?: string;
  audio?: Array<{ scene_index: number; url?: string; duration_sec?: number }>;
}

type Brand = ExplainerStoryboard["meta"]["brand"];
...
function cameraTween(sel: string, motion: string | null | undefined, start: number, dur: number): string {
  switch (motion) {
    case "slow_push_in":
...
// `anim` are GSAP statements appended to the main timeline.
function tplHero(scene: ExplainerScene, b: Brand, id: string, start: number, dur: number): TemplateOutput {
  const html = `
...
function tplScreenshotZoom(
  ...
  const shot = assets.screenshot_url || "";
  const inner = shot
...
function tplBullets(scene: ExplainerScene, b: Brand, id: string, start: number): TemplateOutput {
  ...
  const anim =
    ...
    items
      .map(
        (_, i) =>
          `tl.from("#${id}-li-${i}",{opacity:0,x:-60,duration:0.6,ease:"power3.out"},${(start + 0.5 + i * 0.55).toFixed(2)});`,
      )
      .join("");
  return { html, anim };
}

function tplComparison(scene: ExplainerScene, b: Brand, id: string, start: number): TemplateOutput {
...
function tplStat(scene: ExplainerScene, b: Brand, id: string, start: number): TemplateOutput {
...
function tplCta(scene: ExplainerScene, b: Brand, id: string, start: number): TemplateOutput {
...
const TEMPLATES: Record<
  string,
  (scene: ExplainerScene, b: Brand, id: string, start: number, dur: number, assets: CompositionAssets) => TemplateOutput
> = {
  hero: tplHero,
...
/**
 * Build the full HyperFrames-compatible composition `index.html` for a storyboard.
 * Returns the same HTML string that `build.js`'s `build()` writes to disk — usable
 * directly as an `<iframe srcdoc>` for a live preview. Pure and deterministic.
 */
export function buildCompositionHtml(storyboard: ExplainerStoryboard, assets: CompositionAssets = {}): string {
  const b = storyboard.meta?.brand || ({} as Brand);
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    html,body{width:${COMPOSITION_WIDTH}px;height:${COMPOSITION_HEIGHT}px;overflow:hidden;background:#000;}
    body{font-family:"${b.font || "Inter"}",sans-serif;}
  </style>
</head>
<body>
  <div id="root" data-composition-id="main" data-start="0" data-duration="${total}" data-width="${COMPOSITION_WIDTH}" data-height="${COMPOSITION_HEIGHT}">
${sections.join("\n")}
${audios.join("\n")}
  </div>
  <script>
...
/** Total composition duration in seconds for a storyboard (sum of scene beats,
 *  honoring the same voice-over extension rule as the renderer). */
export function compositionDurationSec(storyboard: ExplainerStoryboard, assets: CompositionAssets = {}): number {
  const scenes = storyboard.scenes || [];
  return scenes.reduce((t, scene, i) => {
    const planned = Math.max(2, Number(scene.duration_sec) || 5);
    const audio = (assets.audio || []).find((a) => a.scene_index === i);
    const dur = audio?.duration_sec
      ? Math.max(planned, Math.ceil((Number(audio.duration_sec) + 0.7) * 10) / 10)
