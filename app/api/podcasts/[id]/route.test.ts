...
describe("PATCH /api/podcasts/[id]", () => {
  ...
  it("persists lipsync_quality_mode without clearing the current render (T-1148b)", async () => {
    let payload: Record<string, unknown> | undefined;
    vi.mocked(getUserFromRequest).mockResolvedValue(USER);
    vi.mocked(createServiceClient).mockReturnValue(
      makeService({
        "podcasts:select": () => ({ data: { id: "p1", user_id: USER.id, metadata: { render_mode: "lipsync_premium" } }, error: null }),
        "podcasts:update": (s) => { payload = s.payload as Record<string, unknown>; return { data: { id: "p1" }, error: null }; },
      }) as never,
    );
    const res = await PATCH(req({ lipsync_quality_mode: "premium" }), ctx("p1"));
    expect(res.status).toBe(200);
    expect(payload?.metadata).toEqual({ render_mode: "lipsync_premium", lipsync_quality_mode: "premium" });
    expect("video_url" in (payload || {})).toBe(false);
    expect("render_status" in (payload || {})).toBe(false);
  });

  it("rejects an invalid lipsync_quality_mode", async () => {
    vi.mocked(getUserFromRequest).mockResolvedValue(USER);
    vi.mocked(createServiceClient).mockReturnValue(
      makeService({ "podcasts:select": () => ({ data: { id: "p1", user_id: USER.id, metadata: {} }, error: null }) }) as never,
    );
    expect((await PATCH(req({ lipsync_quality_mode: "heygen" }), ctx("p1"))).status).toBe(400);
  });
