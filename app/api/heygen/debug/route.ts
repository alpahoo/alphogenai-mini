// TEMPORARY debug — test asset:// with new asset id. Secret-gated.
import { NextResponse } from "next/server";

const URL_ = "https://ark.ap-southeast.bytepluses.com/api/v3/contents/generations/tasks";
const ASSET = "asset://asset-20260607230638-229pt";
...
export async function GET(req: Request) {
  ...
  if (task) {
    ...
    return NextResponse.json({ poll: true, status: r.status, raw: await r.json() });
  }
  ...
  return NextResponse.json({ create: true, status: r.status, raw: await r.json() });
}
